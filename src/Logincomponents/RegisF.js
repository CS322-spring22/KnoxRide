...
const RegisF = () => {
  ...
  const handleSignup = () => {
    clearErrors();
    setLoader(true);
    const { user1 } = auth.createUserWithEmailAndPassword(email, password)
      .catch((err) => {
        switch (err.code) {
  ...
  return (
    <div className='Con'>
      <div className='RegisF'>
        ...
        <input type="number" required minlength="10" maxlength="10" value={phonenumber} onChange={(e) => setPhonenumber(e.target.value)} />
        <p className="errorMsg">{phoneError}</p>

        <label>Car License</label>
        <input type="text" required minlength="7" maxlength="10" value={license} onChange={(e) => setLicense(e.target.value)} />

        <label>Car</label>
        <input type="text" required value={car} onChange={(e) => setCar(e.target.value)} />
